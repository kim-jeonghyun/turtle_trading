...
릴리즈에 포함된 변경사항이 문서에 반영되었는지 확인한다.

다음 항목을 점검하고, 누락이 있으면 경고를 출력한다:

- [ ] `README.md` 프로젝트 구조가 실제 디렉토리와 일치
- [ ] `README.md` "빠른 시작" 섹션이 현재 배포 방식 반영
- [ ] `scripts/README.md` Quick Reference에 신규 스크립트 포함
- [ ] `docs/operations-guide.md` cron 스케줄이 `crontab` 파일과 일치
- [ ] `CHANGELOG.md`에 변경 내역 기록

누락 항목이 있는 경우:
```
경고: 아래 문서가 최신 상태가 아닐 수 있습니다:
- [누락 항목 목록]
릴리즈를 계속하려면 'y'를 입력하세요. 문서를 먼저 수정하려면 'n'을 입력하세요.
```

사용자가 'n'을 입력하면 중단하고 "문서를 수정한 후 다시 릴리즈를 실행하세요."를 출력한다.
