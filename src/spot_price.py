...
class SpotPriceFetcher:
    ...
    async def fetch_spot_price(self, symbol: str) -> Optional[dict[str, Any]]:
        """실시간 현재가 조회.
    ...
    async def _fetch_kr(self, symbol: str) -> Optional[dict[str, Any]]:
        """KR 종목 KIS API 조회."""
        ...
        if not result:  # {} is falsy
            return None
        return {**result, "is_delayed": False}

    async def _fetch_us(self, symbol: str) -> Optional[dict[str, Any]]:
        """US 종목 KIS API 조회."""
        assert self._kis is not None  # caller checks _kis before dispatch
    ...
    async def _fetch_crypto(self, symbol: str) -> Optional[dict[str, Any]]:
    ...
    def _fallback_yfinance(self, symbol: str) -> Optional[dict[str, Any]]:
