...
class TestTrueRange:
    def test_basic_true_range(self):
        ...
        df = pd.DataFrame({"high": [110, 115, 112], "low": [95, 105, 108], "close": [100, 110, 109]})
    ...
    def test_gap_up_true_range(self):
        """갭 상승 시 TR이 |H-PC| 반영"""
        df = pd.DataFrame({"high": [110, 130], "low": [100, 120], "close": [105, 125]})
        tr = calculate_true_range(df)
...
class TestCalculateUnitSize:
    ...
    def test_argument_order(self):
        ...
        result = calculate_unit_size(n_value=5.0, account_equity=100000, risk_per_unit=0.01)
        # 100000 * 0.01 / 5.0 = 200
        assert result == 200
...
class TestAddTurtleIndicators:
    def test_all_columns_present(self, sample_ohlcv_df):
        ...
        required = ["TrueRange", "N", "dc_high_55", "dc_low_55", "dc_high_20", "dc_low_20", "dc_high_10", "dc_low_10"]
        for col in required:
