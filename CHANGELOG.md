...
Pre-Live Safety — 킬 스위치, 포지션 동기화 검증, VI/CB 감지, 데이터 무결성 검증, 일별 OHLCV 수집.

### Added
- `src/kill_switch.py`: 시스템 레벨 킬 스위치 — `config/system_status.yaml` 플래그로 전체 신규 진입 즉시 중단 (#109)
- `scripts/toggle_trading.py`: 킬 스위치 CLI 토글 스크립트 (#109)
- `src/position_sync.py`: `PositionSyncVerifier` — KIS 계좌 실제 포지션 vs `positions.json` 불일치 감지 + 알림 (#125)
- `src/vi_cb_detector.py`: `VICBDetector` — VI(변동성 완화장치)/CB(서킷브레이커) 상태 감지 + 주문 보류 (#127)
- `scripts/collect_daily_ohlcv.py`: 유니버스 종목 일별 OHLCV 수집 — KOSPI/KOSDAQ ~350종목 장마감 후 cron 수집, Parquet 저장 (#103)
- `scripts/validate_data.py`: OHLCV 데이터 무결성 검증 — 논리 일관성, 날짜 갭, 가격 이상치 감지 (#123)
- `docs/roadmap.md`: Agent Council 합의 기반 프로젝트 로드맵 (#121)
- 테스트 추가: 킬 스위치, 포지션 동기화, VI/CB 감지, 데이터 무결성

### Changed
- `src/spot_price.py`: KIS 응답에서 `vi_cls_code` (VI 구분코드) 파싱 추가 (#127)
- `src/auto_trader.py`: `place_order()`에 VI/CB 사전 가드 삽입, `get_daily_stats()`에 REJECTED 카운트 추가 (#127)
- `scripts/check_positions.py`: 시그널 생성 루프에 VI/CB 종목 스킵 로직 (#127)
- `scripts/auto_trade.py`: STATUS_LABELS에 REJECTED 추가 (#127)
- `scripts/health_check.py`: 포지션 동기화 + VI/CB 가드 와이어링 검증 추가 (#125, #127)
- supercronic v0.2.33 → v0.2.43 업그레이드: PID 1 fork exec 버그 수정 (#104)
- `docker-compose.yaml`에 `init: true` 추가 (defense-in-depth)

### Fixed
- `scripts/README.md` 킬 스위치 깨진 앵커 링크 수정 (#119)
- `enforce-branch-policy` 훅에 태그 push 예외 추가 (#107)
- Release v3.6.0 CHANGELOG 링크 보정

### Documentation
- README Docker 배포 가이드 + 프로젝트 구조 업데이트 (#111)
- `scripts/README.md` v2.0 전면 개편 (#112)
- `docs/operations-guide.md` cron 스케줄 문서 추가 (#113)
- 릴리즈 프로세스에 문서 동기화 체크리스트 추가 (#114)
...
[Unreleased]: https://github.com/kim-jeonghyun/turtle_trading/compare/v3.7.0...HEAD
