...
운영 고도화 — Market Intelligence Pipeline, AssetGroup 분리, entry_reason 전파 완성, 로드맵 현행화.

### Added
- PR #203: `src/market_breadth.py` — 시장 브레드스 지표 (% above MA 20/50/200, 52주 NH/NL, AD line, 종합 점수)
- PR #203: `src/regime_detector.py` — Rule-based 시장 레짐 분류 5단계 (SMA50/200 crossover, 1.5% slope threshold)
- PR #203: `src/screener.py` — Strategy Protocol 기반 멀티 전략 스크리너 (확장 가능)
- PR #203: `scripts/market_intelligence.py` — 시장 인텔리전스 리포트 생성·전송 (lock 기반 동시 실행 방지)
- PR #203: `src/data_store.py` bulk API 추가 (`list_accumulated_symbols`, `load_multiple_ohlcv`)
- PR #203: `src/notifier.py` `send_market_intelligence()` 알림 메서드
- PR #203: 62개 신규 테스트 (market_breadth 18, regime_detector 16, screener 16, market_intelligence 12)
- PR #183: `src/types.py` `AssetGroup.US_TECH` enum 추가 (14→15그룹)

### Changed
- PR #199: `pages/*.py` render() 시그니처 `**kwargs` 통일 (7페이지 + app.py)
- PR #199: `src/backtester.py` entry_reason 전파 (backtester 경로)
- PR #205: `docs/roadmap.md` Phase 1 완료 반영, Post-Phase 1 개선 사항 추가
- PR #203: `scripts/collect_daily_ohlcv.py` 수집 완료 후 market_intelligence.py 자동 호출 (subprocess, --date 전파)

### Fixed
- PR #201: `scripts/performance_review.py` `export_trades_csv()`에 `entry_reason` 컬럼 누락 수정
- PR #203: `src/regime_detector.py` docstring slope threshold 0.5%→1.5% 오류 수정

---

## [3.9.0] — 2026-03-10

Live Operations + 유니버스 확장 — 성과 대시보드, 거래 분석, 월간 리포트, 알림 고도화, 42종목 유니버스.
...
- PR #190: `src/analytics.py` 확장 — 종목별 PnL, 에쿼티 커브, 이상 거래 감지
- PR #191: 거래 분석 리포트 — entry_reason, from_dict backward compat, 거래 차트 시각화
- PR #192: `scripts/monthly_report.py` — 월간 성과 요약 + cron 자동화
- PR #193: Streamlit 대시보드 v3.9.0 — app.py 리팩토링, pages/ 패키지, performance/risk 페이지
- PR #194: 알림 고도화 — PnL 일일 요약, 주간 성과 알림, 이상 거래 감지 알림
- PR #196: v3.9.0 릴리즈 — 버전 갱신, operations-guide 업데이트
...
- PR #195: async 버그, CI pages/ 스코프, mypy 수정
...
[Unreleased]: https://github.com/kim-jeonghyun/turtle_trading/compare/v3.9.1...HEAD
[3.9.1]: https://github.com/kim-jeonghyun/turtle_trading/compare/v3.9.0...v3.9.1
[3.9.0]: https://github.com/kim-jeonghyun/turtle_trading/compare/v3.8.1...v3.9.0
